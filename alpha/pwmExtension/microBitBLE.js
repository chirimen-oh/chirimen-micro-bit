/**
  ...
  2020.11.26: https://makecode.microbit.org/_2tDWFtd7pH7x   PWMperiod set "p [portNumb] [period in us]"
  =======================================================================================================
...
(function (window) {
  ...
  var microBitBleFactory = (function () {
    ...
    function getGPIOPort(mbBleUart, portNumb) {
      ...
      async function setPwmPeriod(period) {
        // periodはマイクロ秒[uS]です
        console.log("called setPwmPeriod:", period);
        if (ain && out) {
          var cmd =
            "p" + zeroPadding(pNumber, 2) + zeroPadding(Math.floor(period), 5);
          var returnData = await mbBleUart.sendCmd2MicroBit(cmd);
          if (returnData == null) {
            console.error("mbBLE com error...");
            return;
          ...
          if (debugLog) {
            console.log("GPIO setPwmPeriod:", pNumber, returnData);
          }
          return true;
        } else {
          throw Error("Can't set PWM period. This portis not analog out mode.");
        }
      }
